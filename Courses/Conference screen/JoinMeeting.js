...
function updateVideoLayout() {
  ...
  if (participants.length > 0) {
    ...
    // Make the first participant's video the biggest
    participants[0].classList.add("big-video");

    // Make the rest of the participants' videos smaller
    for (let i = 1; i < participants.length; i++) {
      participants[i].classList.remove("big-video");
      participants[i].classList.add("small-video");
    }
    ...
    for (let i = 0; i < participants.length; i++) {
      participants[i].addEventListener("click", () => {
        if (i !== 0) {
          ...
          // Update video layout after swapping
          updateVideoLayout();
        }
      });
    }
...
toggleWebCamButton.addEventListener("click", async () => {
  if (isWebCamOn) {
    ...
    vElement.style.backgroundSize="100%";
